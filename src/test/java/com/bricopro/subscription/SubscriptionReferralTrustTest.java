...
import com.bricopro.subscription.RecurringTask.Frequency;
import com.bricopro.subscription.RecurringTask.RecurringStatus;
...
class RecurringTaskServiceTest {
    ...
    class Pause {
        ...
        void pausesActive() {
            ...
            assertThat(result.getStatus()).isEqualTo(RecurringStatus.PAUSED);
        }
    ...
    class ProcessRecurringTasks {
        ...
        void createsTaskForDueEntry() {
            ...
            when(recurringRepo.findByStatusAndNextScheduledDateLessThanEqual(
                    RecurringStatus.ACTIVE, LocalDate.now())).thenReturn(List.of(activeRecurring));
        ...
        void advancesNextDateWeekly() {
            ...
            activeRecurring.setFrequency(Frequency.WEEKLY);
        ...
        void advancesNextDateMonthly() {
            LocalDate originalDate = LocalDate.now();
            activeRecurring.setFrequency(Frequency.MONTHLY);
            activeRecurring.setNextScheduledDate(originalDate);
        ...
        void advancesNextDateBiweekly() {
            LocalDate originalDate = LocalDate.now();
            activeRecurring.setFrequency(Frequency.BIWEEKLY);
            activeRecurring.setNextScheduledDate(originalDate);
        ...
        void cancelsWhenPastEndDate() {
            activeRecurring.setNextScheduledDate(LocalDate.now());
            activeRecurring.setEndDate(LocalDate.now().minusDays(1));
            ...
            assertThat(activeRecurring.getStatus()).isEqualTo(RecurringStatus.CANCELLED);
        ...
        void noDueTasksNothingCreated() {
            when(recurringRepo.findByStatusAndNextScheduledDateLessThanEqual(
                    RecurringStatus.ACTIVE, LocalDate.now())).thenReturn(List.of());
